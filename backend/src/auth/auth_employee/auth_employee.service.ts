...
export class AuthEmployeeService {
  ...
  async login(data: LoginAuthDto) {
    const user = await this.employeeService.validateUser(data.username, data.password);
  ...
  async getTokens(userId: string, username: string) {
    const [accessToken, refreshToken] = await Promise.all([
      this.jwtService.signAsync(
        ...
        {
          secret: this.configService.get<string>('JWT_ACCESS_SECRET_EMPLOYEE'),
          expiresIn: this.configService.get("JWT_ACCESS_TOKEN_EXPIRED_EMPLOYEE"),
        },
      ),
      this.jwtService.signAsync(
        {
        ...
        {
          secret: this.configService.get<string>('JWT_REFRESH_SECRET_EMPLOYEE'),
          expiresIn: this.configService.get("JWT_REFRESH_TOKEN_EXPIRED_EMPLOYEE"),
        },
      ),
    ]);
  ...
  async refreshTokens(userId: string, refreshToken: string) {
    ...
    if (!refreshTokenMatches) throw new ForbiddenException('Access for employee Denied');
