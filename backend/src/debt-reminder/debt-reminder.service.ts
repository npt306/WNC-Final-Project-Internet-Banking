...
export class DebtReminderService {
  ...
  async create(createDebtReminderDto: CreateDebtReminderDto): Promise<DebtReminder> {
    const newDebtReminder = this.debtReminderRepository.create(createDebtReminderDto);
  ...
  async findOne(id: string): Promise<DebtReminder> {
    return await this.debtReminderRepository.findOneBy({ _id: new ObjectId(id) }); 
  }

  async findOneDebtReminder(id: string): Promise<DebtReminder> {
    const objectId = new ObjectId(id);
    const debtReminder =  await this.debtReminderRepository.findOneBy( { _id: objectId  });
    if (!debtReminder) {
  ...
  async update(id: string, updateDebtReminderDto: UpdateDebtReminderDto): Promise<DebtReminder> {
    await this.findOneDebtReminder(id);
    await this.debtReminderRepository.update({ _id: new ObjectId(id) }, updateDebtReminderDto);
